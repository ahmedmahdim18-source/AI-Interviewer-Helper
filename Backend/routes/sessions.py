...
def create_session():
    ...
    # Get custom question counts
    behavioral_count = data.get("behavioral_count", 5)
    technical_count = data.get("technical_count", 5)
    
    # Validate counts
    try:
        behavioral_count = max(1, min(20, int(behavioral_count)))
        technical_count = max(1, min(20, int(technical_count)))
    except (ValueError, TypeError):
        behavioral_count, technical_count = 5, 5
